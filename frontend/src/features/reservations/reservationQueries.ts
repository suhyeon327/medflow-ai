...
import {
  ...
  getDoctorReservationPatient,
  getDoctorReservations,
  getPatientReservations,
  updateDoctorReservationStatus,
...
export const reservationKeys = {
  ...
  patient: (filters: ReservationFilters) =>
    ["reservations", "patient", filters] as const,
  schedules: (doctorId: number) => ["doctors", doctorId, "schedules"] as const,
  doctor: (filters: DoctorReservationFilters) =>
    ["reservations", "doctor", filters] as const,
  patientDetail: (reservationId: number) =>
    ["reservations", reservationId, "patient"] as const,
  admin: (filters: AdminReservationFilters) =>
...
export function useDoctorReservationsQuery(filters: DoctorReservationFilters) {
  return useQuery({
    queryKey: reservationKeys.doctor(filters),
    queryFn: () => getDoctorReservations(filters),
  });
}

export function useDoctorReservationPatientQuery(reservationId: number | null) {
  return useQuery({
    queryKey: reservationKeys.patientDetail(reservationId ?? 0),
    queryFn: () => getDoctorReservationPatient(reservationId as number),
...
export function useUpdateDoctorReservationStatusMutation() {
  ...
  return useMutation({
    ...
    }) => updateDoctorReservationStatus(reservationId, status),
    onSuccess: () =>
...
export function useAdminReservationsQuery(filters: AdminReservationFilters) {
  return useQuery({
    queryKey: reservationKeys.admin(filters),
  ...
  return useMutation({
    ...
    onSuccess: async (_, variables) => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: reservationKeys.all }),
        queryClient.invalidateQueries({
          queryKey: reservationKeys.schedules(variables.doctorId),
        }),
      ]);
    },
  });
