...
export function DoctorInfoStep({
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <div>
        <p className="text-sm font-semibold text-blue-700">의사 회원가입</p>
        <h1 className="mt-2 text-2xl font-bold">의사 정보를 입력해 주세요.</h1>
      ...
      <label className="block text-sm font-medium">
        ...
        <input
          ...
          onChange={(event) =>
            onChange({ ...form, licenseNumber: event.target.value })
          }
          className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5"
        />
      </label>
      <label className="block text-sm font-medium">
        ...
        <input
          ...
          onChange={(event) =>
            onChange({ ...form, specialty: event.target.value })
          }
          placeholder="예: 내과"
          className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5"
        />
      ...
      <label className="block text-sm font-medium">
        ...
        <textarea
          ...
          onChange={(event) =>
            onChange({ ...form, introduction: event.target.value })
          }
          className="mt-2 w-full resize-y rounded-md border border-slate-300 px-3 py-2.5"
        />
      </label>
      <label className="block text-sm font-medium">
        연락처
      ...
      <div className="flex gap-3">
        <button
          ...
          onClick={onPrevious}
          disabled={isPending}
          className="flex-1 rounded-md border border-slate-300 px-4 py-2.5 font-semibold text-slate-700 disabled:opacity-60"
        >
          이전
        </button>
        <button
          type="submit"
          disabled={isPending || hospitalsQuery.isPending || !form.hospitalId}
          className="flex-1 rounded-md bg-blue-600 px-4 py-2.5 font-semibold text-white hover:bg-blue-700 disabled:opacity-60"
        >
          {isPending ? "신청 중..." : "가입 및 인증 신청"}
        </button>
      </div>
    </form>
  );
