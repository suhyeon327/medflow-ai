...
export const doctorKeys = {
  ...
  adminList: (status?: string) =>
    ["admin", "doctors", "list", status ?? "all"] as const,
...
export function useOwnDoctorSchedulesQuery(date?: string) {
  return useQuery({
    queryKey: doctorKeys.schedules(date),
    queryFn: () => getOwnDoctorSchedules(date),
  });
}

export function useCreateDoctorSchedulesMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: createDoctorSchedules,
    onSuccess: () =>
...
export function useAdminDoctorsQuery(status?: string) {
  return useQuery({
    queryKey: doctorKeys.adminList(status),
    queryFn: () => getAdminDoctors(status),
  });
}
export function useAdminDoctorQuery(doctorId: number) {
  return useQuery({
    queryKey: doctorKeys.adminDetail(doctorId),
    queryFn: () => getAdminDoctor(doctorId),
    enabled: Number.isInteger(doctorId) && doctorId > 0,
  });
}
...
export function useDoctorDecisionMutation(doctorId: number) {
  ...
  return useMutation({
    mutationFn: (decision: "approve" | "reject") =>
      decision === "approve" ? approveDoctor(doctorId) : rejectDoctor(doctorId),
    onSuccess: async () => {
      await Promise.all([
