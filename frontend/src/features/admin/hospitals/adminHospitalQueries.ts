...
export function useAdminHospitalsQuery() {
  return useQuery({
    queryKey: adminHospitalKeys.all,
...
export function useCreateAdminHospitalMutation() {
  ...
  return useMutation({
    mutationFn: (request: AdminHospitalCreateRequest) =>
      createAdminHospital(request),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: adminHospitalKeys.all }),
  });
}
...
export function useUpdateAdminHospitalMutation() {
  ...
  return useMutation({
    ...
    }) => updateAdminHospital(hospitalId, request),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: adminHospitalKeys.all }),
  });
}
