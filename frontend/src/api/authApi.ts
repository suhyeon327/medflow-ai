import type { LoginRequest, SignupRequest, SignupResponse, TokenResponse, WithdrawRequest, WithdrawResponse } from '../types/auth';
...
export function withdraw(request: WithdrawRequest): Promise<WithdrawResponse> {
  return apiClient<WithdrawResponse>({ url: '/api/v1/auth/withdraw', method: 'DELETE', data: request });
}