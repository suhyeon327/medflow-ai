...
import { ADMIN_HOSPITALS_PATH, ADMIN_RESERVATIONS_PATH, ADMIN_USERS_PATH, DOCTOR_PROFILE_PATH, DOCTOR_RESERVATIONS_PATH, DOCTOR_SCHEDULES_PATH, HOSPITALS_PATH, LOGIN_PATH, PATIENT_PROFILE_PATH, PATIENT_RESERVATIONS_PATH, WITHDRAW_PATH } from '../routes/routePaths';
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {user?.role === 'ADMIN' && (
              <>
                <Link to={ADMIN_HOSPITALS_PATH} className="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">병원 관리</Link>
                <Link to={ADMIN_RESERVATIONS_PATH} className="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">예약 관리</Link>
            ...
            {user?.role === 'DOCTOR' && (
              <>
                <Link to={DOCTOR_PROFILE_PATH} className="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">내 프로필</Link>
                <Link to={DOCTOR_SCHEDULES_PATH} className="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">진료 스케줄</Link>
                <Link to={DOCTOR_RESERVATIONS_PATH} className="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">예약 관리</Link>
              </>
            )}
            {user?.role === 'PATIENT' && (
              <>
