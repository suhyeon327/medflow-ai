...
import { DoctorReservationDetailPage } from "../pages/DoctorReservationDetailPage";
...
function HomeRedirect() {
  const { user } = useAuth();
  return (
    <Navigate to={user ? ROLE_HOME_PATH[user.role] : HOSPITALS_PATH} replace />
  );
}
      ...
      <Route element={<ProtectedRoute />}>
        ...
        <Route element={<AppLayout />}>
          <Route element={<RoleRoute allowedRole="PATIENT" />}>
            <Route
              path={ROLE_HOME_PATH.PATIENT}
              element={<Navigate to={HOSPITALS_PATH} replace />}
            />
            <Route
              path={PATIENT_RESERVATIONS_PATH}
          ...
          <Route element={<RoleRoute allowedRole="DOCTOR" />}>
            <Route
              path={ROLE_HOME_PATH.DOCTOR}
              element={<DoctorDashboardPage />}
            />
            <Route path={DOCTOR_PROFILE_PATH} element={<DoctorProfilePage />} />
            <Route
              path={DOCTOR_SCHEDULES_PATH}
              element={<Navigate to={DOCTOR_RESERVATIONS_PATH} replace />}
            />
            <Route
              path={DOCTOR_RESERVATIONS_PATH}
              element={<DoctorReservationsPage />}
            />
            <Route
              path="/doctor/reservations/:reservationId"
              element={<DoctorReservationDetailPage />}
            />
            <Route
              path="/doctor/questionnaires/:questionnaireId"
              element={<DoctorQuestionnairesPage />}
            />
          </Route>
          <Route element={<RoleRoute allowedRole="ADMIN" />}>
            ...
            <Route path={ADMIN_DOCTORS_PATH} element={<AdminDoctorsPage />} />
            <Route
