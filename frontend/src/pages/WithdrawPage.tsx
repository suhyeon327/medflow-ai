  ...
  return (
    <section className="mx-auto max-w-lg rounded-xl border border-red-200 bg-white p-8 shadow-sm">
      ...
      <div className="mt-5 rounded-md bg-red-50 p-4 text-sm leading-6 text-red-800">
        탈퇴하면 계정과 환자 정보가 삭제 처리되며 현재 로그인도 종료됩니다. 이 작업은 되돌릴 수 없습니다.
      </div>

      <form className="mt-6 space-y-5" onSubmit={handleSubmit}>
        <div>
          <label htmlFor="withdrawPassword" className="mb-2 block text-sm font-medium text-slate-700">현재 비밀번호</label>
          <input
            id="withdrawPassword"
            type="password"
        ...
        <div className="flex gap-3">
          <Link to={ROLE_HOME_PATH.PATIENT} className="flex-1 rounded-md border border-slate-300 px-4 py-2.5 text-center font-semibold text-slate-700 hover:bg-slate-100">취소</Link>
          <button
            type="submit"
