    ...
    <section className="mx-auto max-w-2xl rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold">
          {STATUS_LABEL[profileQuery.data.status]}
        </span>
      </div>
      <form className="mt-8 space-y-5" onSubmit={submit}>
        ...
        <label className="block text-sm font-medium text-slate-700">
          ...
          <select
            ...
            onChange={(e) =>
              setForm({ ...form, hospitalId: Number(e.target.value) })
            }
            className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5"
          >
        ...
        <button
          disabled={updateMutation.isPending || hospitalsQuery.isPending}
          className="w-full rounded-md bg-blue-600 px-4 py-2.5 font-semibold text-white disabled:opacity-60"
        >
