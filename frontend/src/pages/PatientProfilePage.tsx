...
} from "../features/patients/patientProfileQueries";
import type { PatientProfileUpdateRequest } from "../types/patient";
import { WITHDRAW_PATH } from "../routes/routePaths";

const EMPTY_FORM: PatientProfileUpdateRequest = {
  name: "",
  birth: "",
  gender: "MALE",
  phone: "",
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!/^\d{10,11}$/.test(request.phone)) {
      setValidationMessage(
        "전화번호는 하이픈 없이 10~11자리 숫자로 입력해 주세요.",
      );
      return;
    }
  ...
  return (
    <section className="mx-auto max-w-2xl rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
      ...
      <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
        ...
        {(validationMessage || updateMutation.isError) && (
          <p
            role="alert"
            className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700"
          >
            {validationMessage || getApiErrorMessage(updateMutation.error)}
          </p>
        )}
      ...
      <div className="mt-8 border-t border-slate-200 pt-6">
        <Link
          to={WITHDRAW_PATH}
          className="inline-flex rounded-md border border-red-300 px-4 py-2.5 text-sm font-semibold text-red-700 hover:bg-red-50"
        >
          회원 탈퇴
        </Link>
      </div>
