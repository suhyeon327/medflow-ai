...
export function PatientQuestionnairePage() {
  ...
  useEffect(() => {
    if (questionnaire)
      setForm({
        chiefComplaint: questionnaire.chiefComplaint,
        symptomStartedAt: questionnaire.symptomStartedAt.slice(0, 16),
        symptomDescription: questionnaire.symptomDescription,
        painLevel: questionnaire.painLevel,
        temperature: questionnaire.temperature,
        associatedSymptoms: questionnaire.associatedSymptoms ?? "",
      ...
      {questionnaireQuery.isError && !notCreated && (
        <div className="mt-8">
          <QueryError
            ...
            onRetry={() => questionnaireQuery.refetch()}
          />
        </div>
      )}
      {(questionnaire || notCreated) && (
        ...
        >
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <Field label="증상 시작 시점" required>
              <input
                type="datetime-local"
                required
                value={form.symptomStartedAt}
                onChange={(event) =>
                  setForm({ ...form, symptomStartedAt: event.target.value })
                }
                className={INPUT_CLASS}
              />
            </Field>
          </div>
          <Field label="증상 상세 설명" required>
            <textarea
              required
              rows={4}
              value={form.symptomDescription}
              onChange={(event) =>
                setForm({ ...form, symptomDescription: event.target.value })
              }
              className={INPUT_CLASS}
            />
          </Field>
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="통증 정도 (0~10)">
              <input
                type="number"
                min="0"
                max="10"
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <TextField
              ...
              onChange={(value) =>
                setForm({ ...form, associatedSymptoms: value })
              }
            />
            <TextField
              label="과거 병력"
              value={form.medicalHistory}
              onChange={(value) => setForm({ ...form, medicalHistory: value })}
            />
            <TextField
              label="복용 중인 약"
          ...
          <Field label="추가 전달 사항">
            <textarea
              rows={3}
              value={form.additionalNote}
              onChange={(event) =>
                setForm({ ...form, additionalNote: event.target.value })
              }
              className={INPUT_CLASS}
            />
          </Field>
          ...
          {mutation.isSuccess && (
            ...
            >
              문진이 {questionnaire ? "수정" : "등록"}되었습니다.
            </p>
          )}
          <button
            type="submit"
            disabled={mutation.isPending}
            className="rounded-md bg-blue-600 px-5 py-2.5 font-semibold text-white disabled:opacity-60"
          >
            {mutation.isPending
              ? "저장 중..."
              : questionnaire
