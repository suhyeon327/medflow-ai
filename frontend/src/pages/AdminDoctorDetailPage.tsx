  ...
  return (
    <section className="mx-auto max-w-2xl rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-blue-700">의사 인증 관리</p>
          <h1 className="mt-2 text-2xl font-bold">{doctor.doctorName} 의사</h1>
        </div>
      ...
      <dl className="mt-8 grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <dd className="mt-1 font-semibold">{doctor.email}</dd>
        </div>
        <div>
          <dt className="text-sm text-slate-500">면허번호</dt>
          <dd className="mt-1 font-semibold">{doctor.licenseNumber}</dd>
        </div>
        <div>
          <dt className="text-sm text-slate-500">소속 병원</dt>
          <dd className="mt-1 font-semibold">{doctor.hospitalName}</dd>
        </div>
        <div>
          <dt className="text-sm text-slate-500">진료과</dt>
          <dd className="mt-1 font-semibold">{doctor.specialty || "-"}</dd>
        </div>
        <div>
          <dt className="text-sm text-slate-500">연락처</dt>
          <dd className="mt-1 font-semibold">{doctor.contact || "-"}</dd>
        </div>
      </dl>
      ...
      {decisionMutation.isSuccess && (
        <p
          role="status"
          className="mt-5 rounded-md bg-green-50 p-3 text-sm text-green-700"
        >
          {decisionMutation.data.message}
        </p>
      )}
      {doctor.status === "PENDING" && (
        <div className="mt-7 flex gap-3">
          <button
            ...
            className="flex-1 rounded-md border border-red-300 px-4 py-2.5 font-semibold text-red-700 disabled:opacity-60"
          >
            거절
          </button>
          <button
            ...
            className="flex-1 rounded-md bg-blue-600 px-4 py-2.5 font-semibold text-white disabled:opacity-60"
          >
            승인
          </button>
        </div>
